...
[![Docker Repository on Quay](https://quay.io/repository/nycrecords/rhel7-ansible/status "Docker Repository on Quay")](https://quay.io/repository/samdoran/rhel7-ansible)
...
In order to build a new image, set `RHSM_USERNAME` and `RHSM_PASSWORD`as environment variables, or pass them in directly to the the `docker build` command.

    docker build -t rhel7-ansible --build-arg RHSM_USERNAME=$RHSM_USERNAME --build-arg RHSM_PASSWORD=$RHSM_PASSWORD .
...
platforms:
  - name: role-test
    image: "quay.io/nycrecords/${MOLECULE_DISTRIBUTION:-rhel7}-ansible:latest"
